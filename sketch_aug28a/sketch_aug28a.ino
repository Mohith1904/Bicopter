...
int16_t Acc_rawX=0, Acc_rawY=0, Acc_rawZ=0,Gyr_rawX=0, Gyr_rawY=0, Gyr_rawZ=0;
...
float PID, pwmLeft, pwmRight, error=0, previous_error;
float pid_p=0;
float pid_i=0;
float pid_d=0;
/////////////////PID CONSTANTS/////////////////
...
// float avgAccX = sumAccX / 10.0;
//   float avgAccY = sumAccY / 10.0;
//   float avgAccZ = sumAccZ / 10.0;
//   float avgGyrX = sumGyrX / 10.0;
//   float avgGyrY = sumGyrY / 10.0;

 Acc_rawY = sumAccY / 2.0;
 Acc_rawX = sumAccX / 2.0;
Acc_rawZ = sumAccZ /  2.0;
Gyr_rawY = sumGyrY /  2.0;
Gyr_rawX = sumGyrX /  2.0;
    ...
    */
     ...
     *  will calculate the rooth square.*/
     /*---X---*/
    //  Acceleration_angle[0] = atan((avgAccY/16384.0)/sqrt(pow((avgAccX/16384.0),2) + pow((avgAccZ/16384.0),2)))*rad_to_deg;
    Acceleration_angle[0] = atan((Acc_rawY/16384.0)/sqrt(pow((Acc_rawX/16384.0),2) + pow((Acc_rawZ/16384.0),2)))*rad_to_deg;

     /*---Y---*/
    //  Acceleration_angle[1] = atan(-1*(avgAccX/16384.0)/sqrt(pow((avgAccY/16384.0),2) + pow((avgAccZ/16384.0),2)))*rad_to_deg;
   ...
   *part that afects the angles and ofcourse multiply by 0.98 */
   ...
   /*---Y axis angle---*/
   Total_angle[1] = 0.02 *( Gyro_angle[1]) + 0.98*Acceleration_angle[1];
...
// Serial.print("after averaging 10    ") ;
...
right_prop.writeMicroseconds((pwmRight)*0.92);
